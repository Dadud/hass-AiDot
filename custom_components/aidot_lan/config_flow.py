"""Config flow for AiDot LAN — cloud authentication, LAN P2P control."""
...
class AidotLanConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        """Handle the initial step — ask for cloud credentials."""
        errors: dict[str, str] = {}
        if user_input is not None:
            ...
            if not errors:
                return self.async_create_entry(
                    title=f"AiDot LAN · {user_input[CONF_USERNAME]}",
                    data={CONF_LOGIN_INFO: login_info},
                )
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                "description": (
                    "Enter your AiDot cloud account credentials. "
                    "Devices are controlled locally via LAN P2P when reachable; "
